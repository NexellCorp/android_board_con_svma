...
DTB_IMG=${KERNEL_DIR}/arch/arm/boot/dts/s5p4418-avn_ref-rev00.dtb

UBOOT_LOAD_ADDR=0x40007800

if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_BL1}" == "true" ]; then
	build_bl1_s5p4418 ${BL1_DIR}/bl1-${TARGET_SOC} s5p4418 avn ${DEV_PORTNUM}
fi
...
if [ "${BUILD_ALL}" == "true" ] || [ "${BUILD_KERNEL}" == "true" ]; then
	build_kernel ${KERNEL_DIR} ${TARGET_SOC} ${BOARD_NAME} s5p4418_avn_ref_nougat_defconfig ${CROSS_COMPILE}
	test -d ${OUT_DIR} && \
...
if [ -f ${UBOOT_DIR}/u-boot.bin ]; then
	...
	UBOOT_BOOTARGS="console=ttyAMA3,115200n8 loglevel=7 printk.time=1 androidboot.hardware=avn_ref androidboot.console=ttyAMA3 androidboot.serialno=0123456789ABCDEF quiet"
...
make_ext4_recovery_image \
	...
	${KERNEL_DIR}/arch/arm/boot/dts/s5p4418-avn_ref-rev00.dtb \
